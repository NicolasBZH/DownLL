...
// Jeton d'API (extension navigateur) : en-tête x-downll-token.
const API_TOKEN = process.env.DOWNLL_TOKEN || '';
...
function isAuthed(req) {
  ...
  // Jeton d'API (extension) : en-tête x-downll-token.
  const tok = req.headers && req.headers['x-downll-token'];
  if (API_TOKEN && tok && timingEqual(tok, API_TOKEN)) return true;
